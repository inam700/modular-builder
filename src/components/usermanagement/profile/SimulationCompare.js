...
import { faCaretLeft, faCircle, faCross, faEdit, faTimes } from "@fortawesome/free-solid-svg-icons"
...
class SimulationCompare extends Component {
    ...
    if (data.length != 0) {
      if (data.length != 0) {
        ...
        dataHeaderGeneral(type, method).then((result) => {
          ...
          console.log("first call", JSON.parse(responsejson.data.simulation.series));
          if (responsejson.status === "Success") {
            ...
            if (data.length != 2) {
              alert("work")
              window.simulationGraph(Graph1, this.state.xAxisValue, this.state.yAxisValue,this.state.graphType);
              this.setState({
          ...
          if (data.length == 2) {
            ...
            dataHeaderGeneral(type, method).then((result) => {
              let responsejson = result;
              console.log("second call", JSON.parse(responsejson.data.simulation.series));
              if (responsejson.status === "Success") {
                console.log("Second response", responsejson);
                this.setState({
                  simulationData2: JSON.parse(responsejson.data.simulation.buttons),
                ...
                Graph2.map((Detail, i) => {
                  ...
                  if (Detail.name === "IL Range") {
                    return Detail.visible = false, Detail.type = "line", Detail.color = colors[i]
                  } else {
                ...
                window.simulationGraph(this.state.MergeGraphData, this.state.xAxisValue, this.state.yAxisValue,this.state.graphType);
  ...
  downloadPdf = (event) => {
    domtoimage.toJpeg(document.getElementById('container-graph'), { quality: 0.95 })
      ...
      <div className="ml-5">
        <div className="row ml-2 mt-3">
          <div className="col-1 mr-0 p-0  pt-2 w-100 overviewheadings">
            ...
            <FontAwesomeIcon className="Icons"
              style={{ color: '#2A659C"', fontSize: "30px", marginBottom: "15px" }} icon={faCaretLeft}></FontAwesomeIcon>
        ...
        <div className="simulation  pt-2 pb-0 ml-0 mr-0">
          ...
          <div className=" ml-2 pl-3 w-100 mr-0" >
            ...
            <div className="main-section">
              <div className="row w-100" style={this.state.showHeader1 ? {} : { display: "none" }} >
                <div className="col-2" style={{ backgroundColor: '#b1d49d' }}
                  id="btnGridsm">

                  <p style={{ marginTop: "60px", fontSize: "18px", color: "white", textDecoration: "none" }}>
                    {this.state.simName1}</p>
                </div>
                <div className="col-7 d-flex pl-5 pt-2 pr-0" style={{ backgroundColor: '#e7f2e1' }}>
                  <div className="w-100">
                    <div className="row" >
                        ...
                        {this.state.simulationData1.map(s => {
                          if (s.type == 'header') {
                            return (<div style={headerleftright}>
                                  ...
                                  <div className="myIcons" style={iconDisplay}>
                                    {Array.from({ length: s.ports }, (item, index) =>
                                      (index == s.activePort && s.error) ?
                                        <FontAwesomeIcon key={index} className="Icons" style={{ color: '#D00B0B' }} icon={faCircle}></FontAwesomeIcon>
                                        : (index == s.activePort && !s.error) ?
                              ...
                              <p onClick={() => this.openModalHeader(s.id, s.type)}>Header</p>
                            </div>
                            )
                          }
                          else {
                              ...
                              <div className="assembles">
                                ...
                                <div className="assembly">
                                  {(s.cableNo == 1 && s.leftError) ?
                                    <div>
                                      <img className="error-img" src={ErrorImg} alt="" width="20" height="20"></img>
                                      <img src={ErrorLine} alt=""></img>
                                    </div> : (s.cableNo == 1 && !s.leftError) ? <img src={Line} alt=""></img> : <></>}
                                  <div className={"left-header ml-3 widths" + ((s.leftError) ? " error-grid" : "")}>
                                    <div className="myIcons" style={iconDisplay}>
                                      {Array.from({ length: s.leftPorts }, (item, index) =>
                                        (index == s.leftActivePort && s.leftError) ?
                                          <FontAwesomeIcon key={index} className="Icons" style={{ color: '#D00B0B' }} icon={faCircle}></FontAwesomeIcon>
                                          : (index == s.leftActivePort && !s.leftError) ?
                                            <FontAwesomeIcon key={index} className="Icons" style={{ color: '#04C740' }} icon={faCircle}></FontAwesomeIcon>
                                            : <FontAwesomeIcon key={index} className="Icons" style={{ color: '#707070' }} icon={faCircle}></FontAwesomeIcon>
                                  ...
                                  <div className="wire mb-2">
                                    <p className="length-custom">{s.cableLength}m</p>
                                  </div>

                                  <div className={"right-header mr-3 widths" + ((s.rightError) ? " error-grid" : "")}>
                                    <div className="myIcons" style={iconDisplay}>
                                      {Array.from({ length: s.rightPorts }, (item, index) =>
                                        (index == s.rightActivePort && s.rightError) ?
                                          <FontAwesomeIcon key={index} className="Icons" style={{ color: '#D00B0B' }} icon={faCircle}></FontAwesomeIcon>
                                          : (index == s.rightActivePort && !s.rightError) ?
                                            <FontAwesomeIcon key={index} className="Icons" style={{ color: '#04C740' }} icon={faCircle}></FontAwesomeIcon>
                                            : <FontAwesomeIcon key={index} className="Icons" style={{ color: '#707070' }} icon={faCircle}></FontAwesomeIcon>
                                  ...
                                  {(s.rightError) ?
                                    <div>
                                      <img className="error-img" src={ErrorImg} alt="" width="20" height="20"></img>
                                      <img src={ErrorLine} alt=""></img>
                                    </div> : <img src={Line} alt=""></img>}
                                </div>
                                <p className="assembly-heading" onClick={() => this.openModalAssembly(s.id, s.type)}>{s.cableNo}. Assembly</p>
                              </div>
                            )
                          }
                        })}
        ...
        <div className="simulation pt-0 pb-0 pr-2 pt-2 pb-0 ml-0 mr-2">
          ...
          <div className=" w-100 ml-2 pl-3 mr-0 pr-3">
            ...
            <div className="main-section">
              <div className="row" style={this.state.showHeader2 ? {} : { display: "none" }}>
                <div className="col-2" style={{ backgroundColor: '	#d65b59' }}
                  id="btnGridsm">

                  <p style={{ marginTop: "60px", fontSize: "18px", color: "white", textDecoration: "none" }}>
                    {this.state.simName2}</p>
                </div>
                <div className="col-7 d-flex pl-5 pt-2" style={{ backgroundColor: '#f3cdcd' }}>
                  <div className="w-100">
                    <div className="row" >
                      <div className="col-lg-12 w-100 d-flex ">
                                  ...
                                  <div className="myIcons" style={iconDisplay}>
                                    {Array.from({ length: s.ports }, (item, index) =>
                                      (index == s.activePort && s.error) ?
                                        <FontAwesomeIcon key={index} className="Icons" style={{ color: '#D00B0B' }} icon={faCircle}></FontAwesomeIcon>
                                        : (index == s.activePort && !s.error) ?
                                          <FontAwesomeIcon key={index} className="Icons"
                                            style={{ color: '#04C740' }} icon={faCircle}></FontAwesomeIcon>
                                          : <FontAwesomeIcon key={index} className="Icons" style={{ color: '#707070' }} icon={faCircle}></FontAwesomeIcon>
                                    )}
                                  </div>
          ...
          <div className="row pl-2 pt-2">
            <div className="col-1 d-flex"  >
              <img src={plus}></img> <p className="m-0 pt-1 pl-1" style={{ color: " #2A659C" }}>Add link</p>
            </div>
            <div className="col-1 d-flex"  >
                ...
                <div id="accordion">
                  <div className="card mt-3">
                    <div className="card-header" id="headingTwo">
                      <h5 className="mb-0">
                        <button className="button-class" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="true" aria-controls="collapseTwo">
                        ...
                        <div className="row">
                          <div className="col-12 col-adjust">
                            <div style={{ cursor: "pointer" }}
                              className="d-flex justify-content-start align-items-center mt-2 mb-3" onClick={this.takeScreenshot}>
                              <img src={saveicon} ></img>
                              <label>Screenshot</label>
                            </div>
                            <div style={{ cursor: "pointer" }}
                              className="d-flex justify-content-start align-items-center mb-4" onClick={this.downloadPdf}>
                              <img src={pdficon} ></img>
